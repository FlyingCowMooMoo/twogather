...
def populate_dummy_database(boards=None):
    db.database.create_tables([Role, User, UserRoles, EmployeePin, Task, TaskCompletion, MarkedAsTodo, TaskBoard,
                               BoardTask, Comment, TaskComment], True)
    if boards is None:
        raise ValueError('Expected some data')
    for b in boards:
...
def get_dummy_board(tasks=None, manager=None, employees=None, id=None):
